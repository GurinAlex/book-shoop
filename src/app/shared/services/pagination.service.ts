...
export class PaginatorService {
  ...
  initPagination(books: Book[], itemsCount?: number): void {
    this.books = books;
    this.totalList = [];
    ...
    if (books.length > itemsCount || this.itemsPerPage) {
      this.pagesCount = Math.ceil(books.length / itemsCount);
      this.chunkList = this.books.slice(0, this.itemsPerPage);
  ...
  changePage(movie: string, pageNumber?: number | string): void {
    ...
    if (movie && this.currentPage !== Math.ceil(this.books.length / this.itemsPerPage) && movie === 'increase') {
      this.currentPage++;
      this.changeActivePage();
